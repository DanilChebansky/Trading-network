...
class Contacts(models.Model):
    ...
    home = models.CharField(max_length=25, verbose_name="дом", **NULLABLE)
...
class Network(MPTTModel):
    ...
    title = models.CharField(
        max_length=150, verbose_name="название звена сети", choices=title_tuple
    )
    contacts = models.OneToOneField(
        Contacts, on_delete=models.CASCADE, verbose_name="контакты", **NULLABLE
    )
    products = models.ManyToManyField(
        "products.Product", verbose_name="продукты", **NULLABLE
    )
    supplier = TreeForeignKey(
        "self",
    ...
    arrears = models.DecimalField(
        max_digits=50, decimal_places=2, verbose_name="задолженность", **NULLABLE
    )
    creation_time = models.TimeField(
        auto_now_add=True, verbose_name="время создания", **NULLABLE
    )
